...
TMPDOWN=$(mktemp -d)
cd $TMPDOWN
    wget -O 'uboot.zip' 'https://gitlab.com/pine64-org/u-boot/-/jobs/artifacts/universalsuperbox/testing-new-partitions/download?job=build'
...
$SCRIPT/deb-to-bootimg.sh "${TMPDOWN}/linux-image-*-pine64_*.deb" "${TMPDOWN}/initrd.img-touch-arm64" "${TMP}/partitions/boot.img"
